...
------------------------- ----------------------------------------- --------------
: Correlation of Inheritance Factors for Parents and Child {#tbl-inher}

We can also create a link to the table by doing the following: @tbl-inher. If you go back to [Loading and exploring data] and look at the `kable` table, we can create a reference to this max delays table too: @tbl-maxdelays. The addition of the `{#tbl-label}` attribute to the end of the table caption allows us to then make a reference to the table using `@tbl-label`. Note that this reference could appear anywhere throughout the document after the table has appeared.
...
If your thesis has a lot of figures, _Quarto_ might behave better for you than that other word processor. One perk is that it will automatically number the figures accordingly in each chapter. You'll also be able to create a label for each figure, add a caption, and then reference the figure in a way similar to what we saw with tables earlier. If you label your figures, you can move the figures around and _Quarto_ will automatically adjust the numbering for you. No need for you to remember!
...
In the **R** chunk below, we will load in a picture stored as `reed.jpg` in our main directory. We then give it the caption of "Reed logo" and the label "fig-reedlogo". Make note of the different **R** chunk options that are given in the Quarto file (not shown in the rendered document).
...
Here is a reference to the Reed logo: @fig-reedlogo. Note the use of the `fig-` prefix here. By labeling the **R** chunk that contains the figure with `#| label: fig-reedlogo`, we can then reference that figure later as done in the first sentence here. We can also specify the caption for the figure via the chunk option `#| fig-cap:`.
...
Here is a reference to this image: @fig-subd. Note that `#| echo: false` is specified so that the **R** code is hidden in the document.
...
As another example, here is a reference: @fig-subd2.  
...
_Quarto_ uses _pandoc_ (<http://pandoc.org/>) to build its bibliographies. One nice benefit of this is that you won't have to do a second compile to load in references as standard LaTeX requires. To cite references in your thesis (after creating your bibliography database), place the reference name inside square brackets and precede it by the "at" symbol. For example, here's a reference to a book about worrying: [@Molina1994]. This `Molina1994` entry appears in a file called `thesis.bib` in the `bib` folder. This bibliography database file was created by a program called BibTeX. You can call this file something else if you like (look at the `bibliography` setting in `_quarto.yml`) and, by default, it is placed in the `bib` folder.  
...
If you look at the `_quarto.yml` file you can see that we can specify the style of the bibliography by referencing the appropriate csl file. You can download a variety of different style files at <https://www.zotero.org/styles>. Make sure to download the file into the csl folder.
...
If you'd like to see examples of other things in this template, please contact the Data @ Reed team (email <data@reed.edu>) with your suggestions. We love to see people using _Quarto_ for their theses, and are happy to help.
