# Quarto Basics {#rmd-basics}

Here is a brief introduction into using _Quarto_. _Markdown_ is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. _Quarto_ provides the flexibility of _Markdown_ with the implementation of **R** (or Python) input and output. For more details on using _Quarto_ see <https://quarto.org>.

Be careful with your spacing in _Markdown_ documents. While whitespace largely is ignored, it does at times give _Markdown_ signals as to how to proceed. As a habit, try to keep everything left aligned whenever possible, especially as you type a new paragraph. In other words, there is no need to indent basic text in the qmd document (in fact, it might cause your text to do funny things if you do).
...
When you click the **Render** button (or run `quarto render` from the terminal), a document will be generated that includes both content as well as the output of any embedded **R** code chunks within the document. You can embed an **R** code chunk like this (`cars` is a built-in **R** dataset):
...
Note that the `#| echo: false` parameter was added to the code chunk to prevent printing of the **R** code that generated the plot. There are plenty of other ways to add chunk options. More information is available at <https://quarto.org/docs/computations/execution-options.html>.

Another useful chunk option is the setting of `#| cache: true` as you see here. If document rendering becomes time consuming due to long computations or plots that are expensive to generate you can use caching to improve performance. Later in this file, you'll see a way to reference plots created in **R** or external figures.
...
A useful function in the `knitr` package for making nice tables in _Quarto_ is called `kable`. It is much easier to use than manually entering values into a table by copying and pasting values into Excel or LaTeX. This again goes to show how nice reproducible documents can be!

```{r}
#| label: tbl-maxdelays
#| tbl-cap: "Maximum arrival delays (minutes) by airline for Portland departures in 2014 || Maximum delays by airline"
kable(max_delays,
...
#| fig-height: 3
#| fig-width: 6
#| fig-cap: "Arrival delay (minutes) versus departure time for all Portland flights on March 3, 2014. Points in the upper right represent late evening flights that accumulated significant delays throughout the day. || Portland departures, March 3, 2014"
flights %>%
  filter(month == 3, day == 3) %>%
  ggplot(aes(x = dep_time, y = arr_delay)) + geom_point()
```

Use `||` inside `fig-cap` to separate the full caption (shown under the figure) from the short entry shown in the List of Figures. The same works for `tbl-cap` and the List of Tables.

## Additional resources

- _Quarto_ Documentation - <https://quarto.org/docs/guide/>
